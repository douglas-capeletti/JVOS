...
public class JMPI extends Operation {

    public JMPI(int reg1, int reg2, int par) {
        super(reg1, reg2, par);
    }

    @Override
    public void doOperation(Thread thread) {
        thread.setPC(thread.getRegister(this.reg1));
    }

    @Override
    public String toString() {
        return lineHeader(this.getClass().getName()).concat("PC <- R" + this.reg1);
    }
}
